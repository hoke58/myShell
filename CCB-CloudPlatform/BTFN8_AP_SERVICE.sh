#!/bin/sh
######################脚本注释#############################
# 文件名： BTFN8_AP_SERVICE.sh                            #
# 功  能： BTFN8服务起停脚本                              #
###########################################################
VERSION="1"                            #版本号
MODIFIED_TIME="20180523"               #修改时间
DEPLOY_UNION="BTFN8"                #部署单元
EDITER_MAIL="wuji.zh@ccb.com"   #编写人邮箱
###########################################################
v_process=`ps -ef|grep -i mongod|grep -v "grep"|awk '{print $2}'`
MONGO_REPLICA_CFG_PATH=/home/ap/blockchain/mongodb/conf/sharding-shard002.conf
PRIMARY_NODE=`mongo --port 17020 --authenticationDatabase=admin --quiet --eval 'rs.isMaster().ismaster'`
SECONDARY1=`mongo --port 17020 --username=root --password=ccb@#Hope123 --authenticationDatabase=admin --eval 'rs.status().members[1].health'|awk 'NR==4 {print $0}'`
SECONDARY2=`mongo --port 17020 --username=root --password=ccb@#Hope123 --authenticationDatabase=admin --eval 'rs.status().members[2].health'|awk 'NR==4 {print $0}'`
